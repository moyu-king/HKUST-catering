...
class OrderServiceImpl implements OrderService {
    ...
    async addOrder(order: Order, foods: any[]): Promise<boolean> {
        try {
            ...
            return !!await Promise.all([await this.orderDao.insertOnce(order), await this.orderDao.insertOrderFood(bothIds)])
    ...
    async getStudentOrderFoods(orderIds: string[]): Promise<any[]> {
        try {
            this.orderDao = new OrderDaoImpl()
            const promiseArray: Promise<any>[] = orderIds.map(orderId => this.orderDao.findOrderFoodByOrderId(orderId))
    ...
    async getStudentNewOrder(studentId: string): Promise<Order> {
        this.orderDao = new OrderDaoImpl()
        return await this.orderDao.findNewOrderByStudentId(studentId).catch(e => null)
    }

    async getStudentNewOrderFoods(orderId: string): Promise<any[]> {
        this.orderDao = new OrderDaoImpl()
        return await this.orderDao.findOrderFoodByOrderId(orderId).catch(e => null)
    }
