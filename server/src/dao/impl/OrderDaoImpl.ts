...
class OrderDaoImpl implements OderDao {
  ...
  insertOnce(order: Order): Promise<boolean> {
    ...
    this.sql =
      "insert into orders(order_id, student_id, admin_phone, price, create_time, order_type, status,discount) values (?, ?, ?, ?, ?, ?, ?, ?)";
    this.sqlParams = [
      order.order_id,
      order.student_id,
      order.admin_phone,
  ...
  findOrderFoodByOrderId(orderId: string): Promise<any[]> {
    ...
    this.sql =
      "select o_f.number, o_f.order_id, f.* from order_food o_f inner join food f on o_f.food_id = f.food_id where order_id = ?";
  ...
  findNewOrderByStudentId(studentId: string): Promise<Order> {
    ...
    this.sql =
      "select * from orders where student_id = ? order by create_time desc";
    this.sqlParams = [studentId];
    return new Promise((resolve, reject) => {
      this.connection.query(
        this.sql,
  ...
  queryByPageAndDate(
    ...
    this.sql =
      "select * from orders where create_time between ? and ? limit ? offset ?";
  ...
  queryCountByDate(startTime: number, endTime: number): Promise<number> {
    ...
    this.connection.connect();
    this.sql =
      "select count(*) count from orders where create_time between ? and ?";
